...
def sample_user():
    """Sample user data"""
    return {"id": 1, "email": "test@example.com", "username": "testuser", "credits": 1000}
...
class TestHistoryInjection:
    ...
    def test_history_injection_prepends_correctly(self, mock_supabase_client, sample_session):
        ...
        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            # Build up history
            save_chat_message(
                sample_session["id"], "user", "Hello", "gpt-4", 0, sample_session["user_id"]
            )
            save_chat_message(
                sample_session["id"],
                "assistant",
            ...
            save_chat_message(
                sample_session["id"], "user", "How are you?", "gpt-4", 0, sample_session["user_id"]
            )
    ...
    def test_alternating_user_assistant_pattern(self, mock_supabase_client, sample_session):
        ...
        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            ...
            for role, content in conversation:
                save_chat_message(
                    sample_session["id"], role, content, "gpt-4", 10, sample_session["user_id"]
                )
...
class TestEdgeCases:
    ...
    def test_very_long_message_content(self, mock_supabase_client, sample_session):
        ...
        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            ...
            msg = save_chat_message(
                sample_session["id"], "user", long_content, "gpt-4", 0, sample_session["user_id"]
            )
    ...
    def test_special_characters_in_content(self, mock_supabase_client, sample_session):
        ...
        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            ...
            msg = save_chat_message(
                sample_session["id"], "user", special_content, "gpt-4", 0, sample_session["user_id"]
            )
    ...
    def test_zero_and_negative_tokens(self, mock_supabase_client, sample_session):
        ...
        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            msg1 = save_chat_message(
                sample_session["id"], "user", "Test", "gpt-4", 0, sample_session["user_id"]
            )
            msg2 = save_chat_message(
                sample_session["id"],
                "assistant",
...
class TestMultimodalContent:
    ...
    def test_json_string_content(self, mock_supabase_client, sample_session):
        ...
        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            ...
            msg = save_chat_message(
                sample_session["id"], "user", json_content, "gpt-4", 0, sample_session["user_id"]
            )
...
class TestSessionManagement:
    ...
    def test_update_session_title(self, mock_supabase_client, sample_session):
        ...
        from src.db.chat_history import get_chat_session, update_chat_session

        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            success = update_chat_session(
                sample_session["id"], sample_session["user_id"], title="Updated Title"
            )
...
class TestSearch:
    ...
    def test_search_by_message_content(self, mock_supabase_client, sample_user):
        ...
        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            ...
            save_chat_message(
                s1["id"], "user", "How do I use Docker?", "gpt-4", 0, sample_user["id"]
            )
            save_chat_message(
                s2["id"], "user", "What is Kubernetes?", "gpt-4", 0, sample_user["id"]
            )
...
class TestErrorHandling:
    ...
    def test_update_nonexistent_session(self, mock_supabase_client):
        ...
        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            success = update_chat_session(session_id=99999, user_id=1, title="New Title")
...
class TestRealWorldScenarios:
    ...
    def test_full_conversation_flow(self, mock_supabase_client, sample_user):
        ...
        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            ...
            # 2. Add conversation
            save_chat_message(
                session["id"], "user", "How do I read a file?", "gpt-4", 5, sample_user["id"]
            )
            save_chat_message(
                session["id"], "assistant", "Use open() function", "gpt-4", 8, sample_user["id"]
            )
            save_chat_message(
                session["id"], "user", "Can you show an example?", "gpt-4", 4, sample_user["id"]
            )
            save_chat_message(
                session["id"],
                "assistant",
    ...
    def test_multi_session_user(self, mock_supabase_client, sample_user):
        ...
        with patch("src.db.chat_history.get_supabase_client", return_value=mock_supabase_client):
            ...
            save_chat_message(
                python_session["id"], "user", "Python question", "gpt-4", 5, sample_user["id"]
            )
            save_chat_message(
                js_session["id"], "user", "JS question", "gpt-4", 5, sample_user["id"]
            )
            save_chat_message(
                docker_session["id"], "user", "Docker question", "gpt-4", 5, sample_user["id"]
            )
