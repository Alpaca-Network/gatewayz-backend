...
class TestPaymentWebhookReferralIntegration:
    ...
    def test_checkout_completed_triggers_referral_bonus(
        self, mock_notification, mock_stripe_session, supabase_client, test_users_for_webhook
    ):
        """
        ...
        payments = (
            supabase_client.table("payments").select("*").eq("user_id", bob["user_id"]).execute()
        )
    ...
    def test_checkout_below_minimum_no_bonus(
        ...
        assert (
            bob_credits == payment_amount_dollars
        ), f"Bob should have ${payment_amount_dollars}, got ${bob_credits}"
    ...
    def test_second_purchase_no_bonus(
        ...
        assert (
            alice_credits_2nd == alice_credits_1st
        ), f"Alice shouldn't get 2nd bonus: was ${alice_credits_1st}, now ${alice_credits_2nd}"
        print(f"✓ No bonus on second purchase: Alice still at ${alice_credits_2nd}")

    @patch("src.services.payments.stripe.checkout.Session.retrieve")
    def test_payment_without_referral_code(
        self, mock_stripe_session, supabase_client, test_users_for_webhook
    ):
        """
        ...
        charlie_after = (
            supabase_client.table("users").select("credits").eq("id", charlie["user_id"]).execute()
        )
        charlie_credits = float(charlie_after.data[0]["credits"])

        assert (
            charlie_credits == payment_amount_dollars
        ), f"Expected ${payment_amount_dollars}, got ${charlie_credits}"
...
class TestWebhookEdgeCases:
    ...
    def test_webhook_with_missing_metadata(
        self, mock_stripe_session, supabase_client, stripe_service
    ):
