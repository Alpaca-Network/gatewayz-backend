...
class TestAuthEndpoints:
    ...
    def test_auth_endpoint_exists(self, client):
        ...
        response = client.post("/auth", json={"email": "test@example.com", "name": "Test User"})
    ...
    def test_user_balance_endpoint(self, mock_get_user, client, mock_user):
        ...
        # With auth header
        response = client.get(
            "/user/balance", headers={"Authorization": f"Bearer {mock_user['api_key']}"}
        )
...
class TestUserEndpoints:
    ...
    def test_credit_transactions_endpoint(
        self, mock_summary, mock_transactions, mock_get_user, client, mock_user
    ):
        ...
        response = client.get(
            "/user/credit-transactions", headers={"Authorization": f"Bearer {mock_user['api_key']}"}
        )
    ...
    def test_user_monitor_endpoint(
        self, mock_rate_limits, mock_usage, mock_get_user, client, mock_user
    ):
        ...
        response = client.get(
            "/user/monitor", headers={"Authorization": f"Bearer {mock_user['api_key']}"}
        )
...
class TestPaymentEndpoints:
    ...
    def test_stripe_webhook_endpoint_exists(self, client):
        ...
        response = client.post(
            "/api/stripe/webhook", data="test", headers={"stripe-signature": "test"}
        )

        # Webhook endpoint always returns HTTP 200 to Stripe, even on errors
        # This prevents infinite retry loops per Stripe best practices
        assert response.status_code == 200
    ...
    def test_payments_list_endpoint(self, mock_payments, mock_get_user, client, mock_user):
        ...
        response = client.get(
            "/api/stripe/payments", headers={"Authorization": f"Bearer {mock_user['api_key']}"}
        )
...
class TestAPIKeyEndpoints:
    ...
    def test_list_api_keys_endpoint(self, mock_keys, mock_get_user, client, mock_user):
        ...
        response = client.get(
            "/api-keys", headers={"Authorization": f"Bearer {mock_user['api_key']}"}
        )
...
class TestAdminEndpoints:
    ...
    def test_admin_list_users_endpoint(
        self, mock_all_users, mock_get_user, client, mock_admin_user
    ):
        ...
        response = client.get(
            "/admin/users", headers={"Authorization": f"Bearer {mock_admin_user['api_key']}"}
        )
    ...
    def test_admin_add_credits_endpoint(
        self, mock_add_credits, mock_get_user, client, mock_admin_user
    ):
...
class TestChatHistoryEndpoints:
    ...
    def test_chat_sessions_list_endpoint(self, mock_sessions, mock_get_user, client, mock_user):
        """Test GET /chat/sessions endpoint exists"""
        mock_get_user.return_value = mock_user
        mock_sessions.return_value = []

        response = client.get(
            "/chat/sessions", headers={"Authorization": f"Bearer {mock_user['api_key']}"}
        )
    ...
    def test_create_chat_session_endpoint(self, mock_create, mock_get_user, client, mock_user):
