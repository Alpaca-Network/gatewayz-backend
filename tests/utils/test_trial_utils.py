...
class TestValidateTrialExpiration:
    ...
    def test_non_trial_user_passes(self):
        ...
        user = {"id": 1, "subscription_status": "active", "trial_expires_at": None}
    ...
    def test_trial_user_without_expiry_passes(self):
        ...
        user = {"id": 2, "subscription_status": "trial", "trial_expires_at": None}
        # Should not raise any exception
        validate_trial_expiration(user)
    ...
    def test_trial_user_with_future_expiry_passes(self):
        ...
        user = {"id": 3, "subscription_status": "trial", "trial_expires_at": future_date}
        # Should not raise any exception
        validate_trial_expiration(user)
    ...
    def test_trial_user_with_past_expiry_raises_402(self):
        ...
        user = {"id": 4, "subscription_status": "trial", "trial_expires_at": past_date}
    ...
    def test_trial_expiry_with_z_suffix(self):
        ...
        user = {"id": 5, "subscription_status": "trial", "trial_expires_at": past_date}
        with pytest.raises(HTTPException) as exc_info:
            validate_trial_expiration(user)
    ...
    def test_trial_expiry_with_timezone(self):
        ...
        past_date = (datetime.now(UTC) - timedelta(days=1)).isoformat()
        user = {"id": 6, "subscription_status": "trial", "trial_expires_at": past_date}
        with pytest.raises(HTTPException) as exc_info:
            validate_trial_expiration(user)
    ...
    def test_trial_expiry_as_datetime_object(self):
        ...
        user = {"id": 7, "subscription_status": "trial", "trial_expires_at": past_date}
        with pytest.raises(HTTPException) as exc_info:
            validate_trial_expiration(user)
    ...
    def test_trial_expiry_naive_datetime(self):
        ...
        user = {"id": 8, "subscription_status": "trial", "trial_expires_at": past_date}
    ...
    def test_invalid_date_format_logs_warning(self, caplog):
        ...
        user = {"id": 9, "subscription_status": "trial", "trial_expires_at": "invalid-date-format"}
        # Should not raise HTTPException, should log warning
        validate_trial_expiration(user)
        # Verify that warning was logged
        assert any(
            "Failed to parse trial_expires_at" in record.message for record in caplog.records
        )

    def test_empty_subscription_status(self):
        """Test handling of empty subscription status"""
        user = {"id": 10, "subscription_status": "", "trial_expires_at": "2024-01-01T00:00:00Z"}
        # Should not raise any exception
        validate_trial_expiration(user)

    def test_missing_subscription_status(self):
        """Test handling of missing subscription_status key"""
        user = {"id": 11, "trial_expires_at": "2024-01-01T00:00:00Z"}
        # Should not raise any exception
        validate_trial_expiration(user)
