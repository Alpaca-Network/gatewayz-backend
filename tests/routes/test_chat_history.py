    ...
    try:
        ...
        if sessions_result.data:
            ...
            for session in sessions_result.data[:5]:
                print(
                    f"      - ID: {session.get('id')}, User: {session.get('user_id')}, TitleSection: {session.get('title')}, Active: {session.get('is_active')}"
                )
        else:
        ...
        if messages_result.data:
            ...
            for msg in messages_result.data[:5]:
                print(
                    f"      - ID: {msg.get('id')}, Session: {msg.get('session_id')}, Role: {msg.get('role')}"
                )
        else:
        ...
        if users_result.data:
            ...
            for user in users_result.data[:3]:
                print(
                    f"      - ID: {user.get('id')}, Username: {user.get('username', 'N/A')}, Email: {user.get('email')}"
                )
        else:
