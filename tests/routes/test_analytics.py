...
class TestLogEvent:
    ...
    def test_log_event_authenticated_user(
        self, mock_posthog, mock_statsig, client, mock_current_user, valid_event_data
    ):
        ...
        response = client.post("/v1/analytics/events", json=valid_event_data)
    ...
    @patch("src.routes.analytics.statsig_service")
    @patch("src.routes.analytics.posthog_service")
    def test_log_event_anonymous_user(self, mock_posthog, mock_statsig, client, valid_event_data):
    ...
    @patch("src.routes.analytics.statsig_service")
    @patch("src.routes.analytics.posthog_service")
    def test_log_event_with_provided_user_id(self, mock_posthog, mock_statsig, client):
        ...
        app.dependency_overrides[get_current_user] = mock_get_current_user_dep
        response = client.post("/v1/analytics/events", json=event_data)
    ...
    @patch("src.routes.analytics.statsig_service")
    @patch("src.routes.analytics.posthog_service")
    def test_log_event_without_metadata(self, mock_posthog, mock_statsig, client):
        """Test logging event without optional metadata"""
        event_data = {"event_name": "button_click"}
        ...
        mock_statsig.log_event.assert_called_once_with(
            user_id="anonymous", event_name="button_click", value=None, metadata=None
        )

    @patch("src.routes.analytics.statsig_service")
    def test_log_event_statsig_error(self, mock_statsig, client, valid_event_data):
...
class TestLogBatchEvents:
    ...
    def test_log_batch_events_authenticated_user(
        self, mock_posthog, mock_statsig, client, mock_current_user
    ):
        ...
        app.dependency_overrides[get_current_user] = mock_get_current_user_dep
        events_data = [
            {"event_name": "chat_message_sent", "metadata": {"model": "gpt-4"}},
            {"event_name": "page_view", "metadata": {"page": "/dashboard"}},
            {"event_name": "button_click", "value": "export_data"},
        ]

        response = client.post("/v1/analytics/batch", json=events_data)
    ...
    def test_log_batch_events_mixed_user_ids(
        ...
        app.dependency_overrides[get_current_user] = mock_get_current_user_dep
        events_data = [
            {"event_name": "event1", "user_id": "user_a"},
            {"event_name": "event2", "user_id": "user_b"},
            {
                "event_name": "event3"
    ...
    @patch("src.routes.analytics.statsig_service")
    @patch("src.routes.analytics.posthog_service")
    def test_log_batch_events_empty_list(self, mock_posthog, mock_statsig, client):
        ...
        app.dependency_overrides[get_current_user] = mock_get_current_user_dep
        response = client.post("/v1/analytics/batch", json=[])
    ...
    @patch("src.routes.analytics.statsig_service")
    @patch("src.routes.analytics.posthog_service")
    def test_log_batch_events_large_batch(self, mock_posthog, mock_statsig, client):
        """Test logging large batch of events"""
        # Create 100 events
        events_data = [{"event_name": f"event_{i}", "metadata": {"index": i}} for i in range(100)]
    ...
    @patch("src.routes.analytics.statsig_service")
    def test_log_batch_events_partial_failure(self, mock_statsig, client):
        """Test error handling when batch logging fails"""
        mock_statsig.log_event.side_effect = Exception("Batch error")

        events_data = [{"event_name": "event1"}, {"event_name": "event2"}]
...
class TestEventMetadataHandling:
    ...
    @patch("src.routes.analytics.statsig_service")
    @patch("src.routes.analytics.posthog_service")
    def test_event_with_complex_metadata(self, mock_posthog, mock_statsig, client):
        ...
        event_data = {
            ...
            "metadata": {
                ...
                "request": {"method": "POST", "endpoint": "/v1/chat/completions"},
                "response": {"status": 200, "latency_ms": 1234},
            },
        }
    ...
    @patch("src.routes.analytics.statsig_service")
    @patch("src.routes.analytics.posthog_service")
    def test_event_with_array_metadata(self, mock_posthog, mock_statsig, client):
        """Test event with array in metadata"""
        event_data = {
            "event_name": "multi_model_request",
            "metadata": {"models": ["gpt-4", "claude-3", "llama-2"], "count": 3},
        }
...
class TestUserIDPriority:
    ...
    def test_user_id_priority_authenticated_over_provided(
        ...
        app.dependency_overrides[get_current_user] = mock_get_current_user_dep
        event_data = {"event_name": "test_event", "user_id": "should_be_ignored"}
    ...
    @patch("src.routes.analytics.statsig_service")
    @patch("src.routes.analytics.posthog_service")
    def test_user_id_priority_provided_over_anonymous(self, mock_posthog, mock_statsig, client):
        """Test provided user_id takes priority over anonymous"""
        event_data = {"event_name": "test_event", "user_id": "custom_user"}
    ...
    @patch("src.routes.analytics.statsig_service")
    @patch("src.routes.analytics.posthog_service")
    def test_user_id_default_to_anonymous(self, mock_posthog, mock_statsig, client):
...
class TestAnalyticsIntegration:
    ...
    @patch("src.routes.analytics.statsig_service")
    @patch("src.routes.analytics.posthog_service")
    def test_single_and_batch_consistency(self, mock_posthog, mock_statsig, client):
        ...
        # Log single event
        response1 = client.post("/v1/analytics/events", json=event_data)

        # Log batch with same event
        response2 = client.post("/v1/analytics/batch", json=[event_data])
