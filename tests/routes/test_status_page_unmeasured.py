...
def _recent_iso() -> str:
    """A timestamp inside the measurement window, so a row reads as monitored."""
...
class TestModelStatusFormatting:
    ...
    def test_a_view_missing_columns_degrades_instead_of_raising(self):
        ...
        # A row with no check timestamp has never been measured, so it reports
        # unknown rather than a fabricated 0% — being unmeasured is not the same
        # as being down. The point of this test is that it degrades per-field
        # instead of raising; that still holds.
    ...
    def test_a_complete_row_is_formatted_faithfully(self):
        row = {
            ...
            # Must be recent: a row older than the measurement window is
            # reported as unknown, so a hard-coded date would rot this test
            # into asserting the stale path by accident.
            "last_called_at": _recent_iso(),
            "last_success_at": _recent_iso(),
