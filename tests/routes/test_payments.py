...
def mock_current_user():
    ...
    return {"id": "1", "username": "testuser", "email": "test@example.com", "is_admin": False}


@pytest.fixture
def mock_admin_user():
    """Sample admin user"""
    return {"id": "99", "username": "admin", "email": "admin@example.com", "is_admin": True}
...
class TestStripeWebhooks:
    ...
    async def test_webhook_success(self, mock_stripe_service, client, mock_webhook_result):
    ...
    async def test_webhook_missing_signature(self, client):
        ...
        response = client.post(
            "/api/stripe/webhook", content=b'{"type": "checkout.session.completed"}'
        )
    ...
    @patch("src.routes.payments.stripe_service")
    async def test_webhook_invalid_signature(self, mock_stripe_service, client):
...
class TestCheckoutSessions:
    ...
    def test_create_checkout_session_success(
        self, mock_get_user, mock_stripe_service, client, mock_current_user, mock_checkout_session
    ):
        ...
        response = client.post("/api/stripe/checkout-session", json=request_data)
    ...
    def test_create_checkout_session_validation_error(
        self, mock_get_user, mock_stripe_service, client, mock_current_user
    ):
...
class TestPaymentIntents:
    ...
    def test_create_payment_intent_success(
        self, mock_get_user, mock_stripe_service, client, mock_current_user, mock_payment_intent
    ):
        ...
        response = client.post("/api/stripe/payment-intent", json=request_data)
...
class TestCreditPackages:
    ...
    @patch("src.routes.payments.stripe_service")
    def test_get_credit_packages_success(self, mock_stripe_service, client):
...
class TestRefunds:
    ...
    def test_create_refund_as_admin_success(
        self, mock_get_user, mock_stripe_service, client, mock_admin_user
    ):
        ...
        response = client.post("/api/stripe/refund", json=request_data)
    ...
    @patch("src.routes.payments.get_current_user")
    def test_create_refund_as_non_admin_fails(self, mock_get_user, client, mock_current_user):
        """Test non-admin cannot create refund"""
        mock_get_user.return_value = mock_current_user

        request_data = {"payment_intent_id": "pi_456", "amount": 1000}
...
class TestPaymentHistory:
    ...
    def test_get_payment_history_success(
        self, mock_get_user, mock_get_payments, client, mock_current_user, mock_payments
    ):
    ...
    def test_get_payment_details_success(
        self, mock_get_user, mock_get_payment, client, mock_current_user, mock_payments
    ):
    ...
    def test_get_payment_details_not_found(
        self, mock_get_user, mock_get_payment, client, mock_current_user
    ):
