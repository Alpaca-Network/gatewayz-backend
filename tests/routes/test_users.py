...
class TestUserBalance:
    ...
    def test_get_balance_active_user(
        self, mock_validate_trial, mock_get_user, client, mock_api_key, mock_user
    ):
        """Test getting balance for active (non-trial) user"""
        mock_get_user.return_value = mock_user
        mock_validate_trial.return_value = {"is_trial": False}

        response = client.get("/user/balance", headers={"Authorization": f"Bearer {mock_api_key}"})
    ...
    def test_get_balance_trial_user(
        self, mock_validate_trial, mock_get_user, client, mock_api_key, mock_trial_user
    ):
    ...
    @patch("src.routes.users.get_user")
    def test_get_balance_invalid_api_key(self, mock_get_user, client):
        """Test balance retrieval with invalid API key"""
        mock_get_user.return_value = None

        response = client.get("/user/balance", headers={"Authorization": "Bearer invalid-key"})
...
class TestUserMonitor:
    ...
    def test_user_monitor_success(
        ...
        response = client.get("/user/monitor", headers={"Authorization": f"Bearer {mock_api_key}"})
    ...
    def test_user_monitor_failed_metrics(
        self, mock_get_metrics, mock_get_user, client, mock_api_key, mock_user
    ):
    ...
    @patch("src.routes.users.get_user")
    def test_user_monitor_invalid_key(self, mock_get_user, client):
        """Test monitoring with invalid API key"""
        mock_get_user.return_value = None

        response = client.get("/user/monitor", headers={"Authorization": "Bearer invalid-key"})
...
class TestRateLimits:
    ...
    def test_get_rate_limits_configured(
        ...
        response = client.get("/user/limit", headers={"Authorization": f"Bearer {mock_api_key}"})
    ...
    def test_get_rate_limits_default(
        self, mock_get_limits, mock_get_user, client, mock_api_key, mock_user
    ):
    ...
    @patch("src.routes.users.get_user")
    def test_get_rate_limits_invalid_key(self, mock_get_user, client):
        """Test rate limits with invalid API key"""
        mock_get_user.return_value = None

        response = client.get("/user/limit", headers={"Authorization": "Bearer invalid-key"})
...
class TestUserProfile:
    ...
    def test_get_profile_success(
        self, mock_get_profile, mock_get_user, client, mock_api_key, mock_user, mock_user_profile
    ):
        """Test successfully getting user profile"""
        mock_get_user.return_value = mock_user
        mock_get_profile.return_value = mock_user_profile

        response = client.get("/user/profile", headers={"Authorization": f"Bearer {mock_api_key}"})
    ...
    def test_get_profile_with_max_tier_tiered_credits(
        self, mock_get_profile, mock_get_user, client, mock_api_key, mock_user
    ):
    ...
    def test_get_profile_tier_display_names(
        ...
        # Test MAX tier
        mock_user_max = {**mock_user, "tier": "max"}
        mock_profile_max = {**mock_user_profile, "tier": "max", "tier_display_name": "MAX"}
        mock_get_user.return_value = mock_user_max
        mock_get_profile.return_value = mock_profile_max
    ...
    def test_update_profile_success(
        ...
        response = client.put(
            "/user/profile", json=update_data, headers={"Authorization": f"Bearer {mock_api_key}"}
        )
    ...
    @patch("src.routes.users.get_user")
    def test_update_profile_no_fields(self, mock_get_user, client, mock_api_key, mock_user):
        """Test updating profile with no fields provided"""
        mock_get_user.return_value = mock_user

        response = client.put(
            "/user/profile", json={}, headers={"Authorization": f"Bearer {mock_api_key}"}
        )
    ...
    def test_update_profile_failed(
        self, mock_update_profile, mock_get_user, client, mock_api_key, mock_user
    ):
...
class TestAccountDeletion:
    ...
    def test_delete_account_success(
        self, mock_delete_account, mock_get_user, client, mock_api_key, mock_user
    ):
        """Test successfully deleting account"""
        mock_get_user.return_value = mock_user
        mock_delete_account.return_value = True

        delete_data = {"confirmation": "DELETE_ACCOUNT"}
    ...
    def test_delete_account_wrong_confirmation(
        self, mock_get_user, client, mock_api_key, mock_user
    ):
    ...
    @patch("src.routes.users.get_user")
    def test_delete_account_invalid_key(self, mock_get_user, client):
...
class TestCreditTransactions:
    ...
    def test_get_transactions_success(
        ...
        response = client.get(
            "/user/credit-transactions", headers={"Authorization": f"Bearer {mock_api_key}"}
        )
    ...
    @patch("src.routes.users.get_user")
    def test_get_transactions_invalid_key(self, mock_get_user, client):
        """Test transactions with invalid API key"""
        mock_get_user.return_value = None

        response = client.get(
            "/user/credit-transactions", headers={"Authorization": "Bearer invalid-key"}
        )
    ...
    def test_get_transactions_error_handling(
        self, mock_get_transactions, mock_get_user, client, mock_api_key, mock_user
    ):
...
class TestUserIntegration:
    ...
    def test_complete_user_info_flow(
        ...
        balance_response = client.get(
            "/user/balance", headers={"Authorization": f"Bearer {mock_api_key}"}
        )
        ...
        monitor_response = client.get(
            "/user/monitor", headers={"Authorization": f"Bearer {mock_api_key}"}
        )
        ...
        limits_response = client.get(
            "/user/limit", headers={"Authorization": f"Bearer {mock_api_key}"}
        )
    ...
    def test_profile_update_workflow(
        ...
        get_response = client.get(
            "/user/profile", headers={"Authorization": f"Bearer {mock_api_key}"}
        )
