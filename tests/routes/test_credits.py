...
pytestmark = pytest.mark.skipif(
    ...
    reason="Credits route tests have Python 3.10 compatibility issues"
)
...
class TestCreditsAdd:
    ...
    def test_add_credits_success(self, mock_log_transaction, mock_supabase, client, admin_user, auth_headers):
...
class TestCreditsAdjust:
    ...
    @patch('src.routes.credits.get_supabase_client')
    @patch('src.routes.credits.log_credit_transaction')
    def test_adjust_credits_positive(self, mock_log_transaction, mock_supabase, client, auth_headers):
...
class TestCreditsRefund:
    ...
    @patch('src.routes.credits.get_supabase_client')
    @patch('src.routes.credits.log_credit_transaction')
    def test_refund_credits_success(self, mock_log_transaction, mock_supabase, client, auth_headers):
...
class TestCreditsTransactions:
    ...
    def test_get_transactions_with_filters(self, mock_get_transactions, client, auth_headers):
        ...
        response = client.get(
            '/credits/transactions?user_id=2&transaction_type=refund&limit=10',
            headers=auth_headers
        )
    ...
    def test_get_transactions_invalid_direction(self, client, auth_headers):
        ...
        response = client.get(
            '/credits/transactions?direction=invalid',
            headers=auth_headers
        )
