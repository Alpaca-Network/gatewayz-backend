    ...
    return [
        {
            ...
            "metadata": {"key_name": "Production API Key", "environment": "live"},
        },
        {
            "id": 2,
            "user_id": 123,
            ...
            "ip_address": "192.168.1.1",
            "metadata": {"endpoint": "/v1/chat/completions", "model": "gpt-4"},
        },
        {
            "id": 3,
            "user_id": 123,
            ...
            "metadata": {"old_key_id": 456, "new_key_id": 457},
        },
    ]
...
class TestGetAuditLogs:
    ...
    def test_get_audit_logs_success(
        ...
        response = client.get(
            "/user/api-keys/audit-logs", headers={"Authorization": "Bearer test_api_key"}
        )
        ...
        mock_get_logs.assert_called_once_with(
            user_id=123, key_id=None, action=None, start_date=None, end_date=None, limit=100
        )
    ...
    def test_get_audit_logs_with_key_filter(
        ...
        response = client.get(
            "/user/api-keys/audit-logs?key_id=456", headers={"Authorization": "Bearer test_api_key"}
        )
    ...
    def test_get_audit_logs_with_limit(
        ...
        response = client.get(
            "/user/api-keys/audit-logs?limit=2", headers={"Authorization": "Bearer test_api_key"}
        )
    ...
    def test_get_audit_logs_empty(
        self, mock_get_user, mock_validate_perms, mock_get_logs, client, mock_user_data
    ):
...
class TestAuditAuthentication:
    ...
    @patch("src.routes.audit.get_user")
    def test_get_audit_logs_invalid_api_key(self, mock_get_user, client):
        """Test retrieving audit logs with invalid API key"""
        mock_get_user.return_value = None

        response = client.get(
            "/user/api-keys/audit-logs", headers={"Authorization": "Bearer invalid_key"}
        )
    ...
    def test_get_audit_logs_insufficient_permissions(
        self, mock_get_user, mock_validate_perms, client, mock_user_data
    ):
        ...
        mock_validate_perms.assert_called_once_with("test_api_key", "read", "api_keys")
...
class TestAuditErrorHandling:
    ...
    @patch("src.routes.audit.get_user")
    def test_get_audit_logs_user_lookup_error(self, mock_get_user, client):
...
class TestAuditIntegration:
    ...
    def test_audit_log_pagination(
        ...
        mock_get_logs.return_value = mock_audit_logs
        response2 = client.get(
            "/user/api-keys/audit-logs?limit=100", headers={"Authorization": "Bearer test_api_key"}
        )
