...
class TestAdminUsersStats:
    ...
    def test_stats_returns_accurate_totals_not_limited(self, mock_client, mock_auth):
        ...
        mock_supabase.table.return_value.select.return_value.ilike.return_value.eq.return_value.execute.return_value = mock_count_result
    ...
    def test_stats_with_filters(self, mock_client, mock_auth):
        ...
        response = client.get("/admin/users/stats?email=gmail", headers={"Authorization": "Bearer mock_token"})
...
class TestAdminUsersGrowth:
    ...
    def test_growth_returns_correct_data_points(self, mock_client, mock_auth):
        ...
        mock_supabase.table.return_value.select.return_value.gte.return_value.lte.return_value.order.return_value.execute.return_value = mock_growth_result
        ...
        mock_supabase.table.return_value.select.return_value.lt.return_value.execute.return_value = mock_before_result
        
        # Make request for 30 days
        response = client.get("/admin/users/growth?days=30", headers={"Authorization": "Bearer mock_token"})
    ...
    def test_growth_different_day_ranges(self, mock_client, mock_auth):
        ...
        for days in [7, 30, 90, 365]:
            response = client.get(f"/admin/users/growth?days={days}", headers={"Authorization": "Bearer mock_token"})
    ...
    def test_growth_invalid_day_range(self, mock_client, mock_auth):
        ...
        for invalid_days in [0, -1, 366]:
            response = client.get(f"/admin/users/growth?days={invalid_days}", headers={"Authorization": "Bearer mock_token"})
