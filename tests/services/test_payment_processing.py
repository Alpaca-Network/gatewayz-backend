...
class TestCheckoutCompletedSubscriptionStatus:
    ...
    @patch('src.services.payments.update_payment_status')
    @patch('src.config.supabase_config.get_supabase_client')
    def test_checkout_completed_syncs_api_keys_when_active_status_with_basic_tier(
        self,
        mock_get_supabase_client,
        mock_update_payment,
        mock_add_credits,
        ...
        """Test that both users and api_keys_new tables are updated when user has active status but basic tier.

        This edge case tests the data inconsistency scenario where a user has:
        - subscription_status='active' (incorrect)
        - tier='basic' (correct for pay-per-use)

        Both tables should be updated to 'inactive' to maintain consistency.
        """
        ...
        session.id = 'cs_test_inconsistent_user'
        session.payment_intent = 'pi_test_inconsistent_user'
        session.amount_total = 500
        session.amount_subtotal = None
        session.currency = 'usd'
        ...
        # User has inconsistent data: active status but basic tier
        mock_client.table().select().eq().execute.return_value = Mock(
            data=[{'subscription_status': 'active', 'tier': 'basic'}]
        )
        mock_client.table().update().eq().execute.return_value = Mock(data=[{}])

        stripe_service._handle_checkout_completed(session)

        # Verify that subscription_status is updated to 'inactive' in BOTH tables
        update_calls = mock_client.table.return_value.update.call_args_list

        # Find all updates with subscription_status='inactive'
        inactive_updates = []
        for call in update_calls:
            if call.args:
                update_data = call.args[0]
                ...
                continue
            if isinstance(update_data, dict) and update_data.get('subscription_status') == 'inactive':
                inactive_updates.append(update_data)

        # Should have at least 2 updates with subscription_status='inactive'
        # (one for users table, one for api_keys_new table)
        assert len(inactive_updates) >= 2, \
            f"Expected at least 2 updates with subscription_status='inactive' " \
            f"(users + api_keys_new tables), but found {len(inactive_updates)}. " \
            f"All update calls: {update_calls}"

        # Verify the api_keys_new update includes is_trial=False and trial_converted=True
        found_api_key_update = False
        for update_data in inactive_updates:
            if update_data.get('is_trial') is False and update_data.get('trial_converted') is True:
                found_api_key_update = True
                break
        assert found_api_key_update, \
            f"Expected api_keys_new to be updated with is_trial=False, " \
            f"trial_converted=True, and subscription_status='inactive'. " \
            f"Updates found: {inactive_updates}"
