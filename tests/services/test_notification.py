...
from src.schemas.notification import LowBalanceAlert, NotificationPreferences, TrialExpiryAlert
...
def mock_user_data():
    """Sample user data"""
    return {"id": 1, "email": "test@example.com", "credits": 100.0, "api_key": "test_api_key_123"}
...
class TestUserPreferences:
    ...
    def test_get_user_preferences_exists(self, notification_service, mock_preferences_data):
        ...
        notification_service.supabase.table.return_value.select.return_value.eq.return_value.execute.return_value = (
            mock_result
        )
    ...
    def test_create_user_preferences_success(self, notification_service):
        ...
        notification_service.supabase.table.return_value.insert.return_value.execute.return_value = (
            mock_result
        )
    ...
    def test_update_user_preferences_success(self, notification_service):
        ...
        mock_result = Mock()
        mock_result.data = [{"user_id": 1}]
        notification_service.supabase.table.return_value.update.return_value.eq.return_value.execute.return_value = (
            mock_result
        )

        updates = {"email_notifications": False, "low_balance_threshold": 5.0}
...
class TestLowBalanceAlerts:
    ...
    def test_check_low_balance_alert_triggered(
        ...
        mock_prefs_result.data = [
            {"user_id": 1, "email_notifications": True, "low_balance_threshold": 10.0}
        ]
    ...
    def test_check_low_balance_alert_trial_user(
        ...
        mock_validate_trial.return_value = {"is_trial": True, "trial_end_date": trial_end}
    ...
    def test_check_low_balance_alert_notifications_disabled(
        self, notification_service, mock_user_data
    ):
        ...
        mock_prefs_result = Mock()
        mock_prefs_result.data = [{"user_id": 1, "email_notifications": False}]
    ...
    def test_check_low_balance_alert_recent_notification(
        ...
        mock_chain = (
            notification_service.supabase.table.return_value.select.return_value.eq.return_value
        )
...
class TestRecentNotificationTracking:
    ...
    def test_has_recent_notification_found(self, notification_service):
        ...
        notification_service.supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.gte.return_value.execute.return_value = (
            mock_result
        )
...
class TestNotificationIntegration:
    ...
    def test_low_balance_alert_with_plan_info(
        self, mock_get_plan, mock_validate_trial, notification_service, mock_user_data
    ):
...
class TestEmailValidation:
    ...
    def test_malformed_emails_rejected(self, notification_service):
        ...
        for email in invalid_emails:
            if email is None:
                ...
                assert (
                    notification_service._is_valid_email(email) is False
    ...
    def test_send_email_skips_invalid_addresses(self, mock_resend, notification_service):
        ...
        result = notification_service.send_email_notification(
            to_email="did:privy:abc123@privy.user", subject="Test", html_content="<p>Test</p>"
        )
    ...
    def test_send_email_proceeds_for_valid_addresses(self, mock_resend, notification_service):
        ...
        result = notification_service.send_email_notification(
            to_email="valid@example.com", subject="Test", html_content="<p>Test</p>"
        )
