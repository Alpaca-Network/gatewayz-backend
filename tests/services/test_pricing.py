...
def test_calculate_cost_happy(monkeypatch, mod):
    ...
    # 10000 per 1M tokens = $0.00001 per token
    # 20000 per 1M tokens = $0.00002 per token
    monkeypatch.setattr(
        mod, "get_model_pricing",
        lambda model_id: {"prompt": 10000, "completion": 20000, "found": True}
    )
    cost = mod.calculate_cost("any/model", prompt_tokens=1000, completion_tokens=500)
    # (1000 * 10000 + 500 * 20000) / 1_000_000 = (10_000_000 + 10_000_000) / 1_000_000 = 0.02
