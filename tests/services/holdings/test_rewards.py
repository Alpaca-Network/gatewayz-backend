...
class FakeHoldingsDB:
    ...
    def __init__(self):
        ...
        self.min_usd: dict[tuple[str, str], Decimal | None] = {}
    ...
    def list_wallets_with_snapshots_for_date(self, day):
        return sorted(
            {w for (w, d) in self.min_usd if d == day.isoformat()},
        )

    def get_min_usd_for_date(self, wallet_address, day):
        return self.min_usd.get((wallet_address.lower(), day.isoformat()))
    ...
    def observe(self, address, usd, day=DAY):
        self.min_usd[(address.lower(), day.isoformat())] = Decimal(str(usd))
...
class TestGlobalBudget:
    ...
    def test_budget_is_spent_in_a_deterministic_wallet_order(self, db, monkeypatch):
        monkeypatch.setattr(
        ...
        for i, wallet in enumerate((W3, W2, W1)):
            ...
            db.link(wallet, 10 + i)
        rewards.run_holdings_rewards_once(DAY)
        # Ascending wallet address, the order list_wallets_with_snapshots_
        # for_date already returns -- the lowest address wins the budget.
        assert list(db.accruals) == [(W1, DAY_STR)]
...
class TestPayoutAndIdempotency:
    ...
    def test_one_failing_wallet_does_not_abort_the_run(self, db, monkeypatch):
        ...
        real_basis = db.get_min_usd_for_date
        ...
        monkeypatch.setattr(rewards, "get_min_usd_for_date", exploding_basis)
        result = rewards.run_holdings_rewards_once(DAY)
