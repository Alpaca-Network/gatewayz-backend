...
class TestSubscriptionUsageCreditsAllocation:
    ...
    def test_max_subscription_monthly_renewal(
        ...
        for month in range(3):
            ...
            call_args = mock_add_credits.call_args
            assert (
                call_args.kwargs["credits"] == 150.0
            ), f"Month {month + 1} should add $150 credits"

    @patch("src.services.payments.add_credits_to_user")
    @patch("stripe.Subscription.retrieve")
    def test_non_subscription_invoice_skipped(
        self, mock_retrieve_subscription, mock_add_credits, stripe_service
    ):
        """Test that non-subscription invoices don't add credits"""
        # Setup - invoice without subscription
        non_sub_invoice = Mock(
            id="in_one_time_789", subscription=None, amount_paid=1000  # No subscription
        )
    ...
    def test_unknown_tier_no_credits(
        self, mock_retrieve_subscription, mock_add_credits, mock_get_credits, stripe_service
    ):
        ...
        unknown_invoice = Mock(id="in_unknown_999", subscription="sub_unknown_999")
...
class TestSubscriptionCreditsIntegration:
    ...
    def test_multiple_users_different_tiers(
        ...
        # User 1: PRO
        pro_sub = Mock(id="sub_pro_1", metadata={"user_id": "100", "tier": "pro"})
        pro_invoice = Mock(id="in_pro_1", subscription="sub_pro_1")

        # User 2: MAX
        max_sub = Mock(id="sub_max_2", metadata={"user_id": "200", "tier": "max"})
        max_invoice = Mock(id="in_max_2", subscription="sub_max_2")

        # User 3: PRO
        pro_sub_2 = Mock(id="sub_pro_3", metadata={"user_id": "300", "tier": "pro"})
...
class TestSubscriptionCreditsEdgeCases:
    ...
    def test_missing_user_id_in_subscription_metadata(
        ...
        bad_sub = Mock(id="sub_bad_123", metadata={"tier": "pro"})  # Missing user_id
    ...
    def test_tier_defaults_to_pro_when_missing(
        ...
        sub_no_tier = Mock(id="sub_no_tier_123", metadata={"user_id": "1"})  # Missing tier
    ...
    def test_case_sensitivity_of_tier(
        ...
        # Test uppercase tier
        sub_upper = Mock(id="sub_upper_123", metadata={"user_id": "1", "tier": "PRO"})  # Uppercase
        ...
        mock_add_credits.reset_mock()
        sub_lower = Mock(id="sub_lower_456", metadata={"user_id": "1", "tier": "pro"})  # Lowercase
...
class TestTrialStatusClearedOnSubscription:
    ...
    def test_subscription_created_clears_trial_status(
        self, mock_get_supabase, stripe_service, mock_max_subscription
    ):
    ...
    def test_subscription_updated_to_active_clears_trial_status(
        self, mock_get_supabase, stripe_service, mock_pro_subscription
    ):
    ...
    def test_subscription_updated_to_canceled_does_not_clear_trial(
        self, mock_get_supabase, stripe_service
    ):
