        ...
        class _RPCShim:
            ...
            def execute(self):
                if self.fn_name == "get_user_usage_metrics":
                    ...
                    usage = [
                        r for r in self.outer.tables["usage_records"] if r.get("api_key") == api_key
                    ]
                    ...
                    tokens_today = sum(
                        r.get("tokens_used", 0) for r in usage if after(r.get("timestamp"), today)
                    )
                    cost_today = sum(
                        r.get("cost", 0.0) for r in usage if after(r.get("timestamp"), today)
                    )
                    requests_this_month = len(
                        [r for r in usage if after(r.get("timestamp"), month)]
                    )
                    tokens_this_month = sum(
                        r.get("tokens_used", 0) for r in usage if after(r.get("timestamp"), month)
                    )
                    cost_this_month = sum(
                        r.get("cost", 0.0) for r in usage if after(r.get("timestamp"), month)
                    )
                    ...
                    if usage:
                        counts = {}
                        for r in usage:
                            counts[r.get("model", "unknown")] = (
                                counts.get(r.get("model", "unknown"), 0) + 1
                            )
...
def sb(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: ("gw_live_primary_TESTKEY_1234567890abcdefghijklm", 1),
    )
...
def test_get_user_prefers_new_api_keys_then_legacy(sb):
    ...
    new_key = {
        "id": 10,
        "api_key": "gw_live_primary_TESTKEY_1234567890abcdefghijklm",
        "user_id": 7,
        "key_name": "Primary",
...
def test_add_credits_and_deduct_credits(sb):
    ...
    users.add_credits_to_user(
        user_id=22, credits=2.0, transaction_type="admin_credit", description="top-up"
    )
...
def test_record_usage_and_metrics(sb):
    ...
    # user + key
    sb.table("users").insert(
        {"id": 77, "username": "m", "email": "m@x.com", "credits": 5}
    ).execute()
...
def test_admin_monitor_data_deduplication_api_key_only(sb):
    ...
    assert (
        out["system_usage_metrics"]["tokens_today"] == 200
...
def test_update_and_get_user_profile(sb):
    ...
    sb.table("users").insert(
        {"id": 9, "username": "z", "email": "z@x.com", "credits": 4, "api_key": "k9"}
    ).execute()
...
def test_get_user_profile_returns_credits_in_cents(sb):
    ...
    assert (
        prof["subscription_allowance"] == 15000
    ), "subscription_allowance should be in cents (150.0 * 100)"
...
def test_get_user_profile_pro_tier_allowance_in_cents(sb):
    ...
    assert (
        prof["subscription_allowance"] == 1500
    ), "Pro tier allowance should be 1500 cents ($15.00)"
...
def test_get_user_profile_max_tier_allowance_in_cents(sb):
    ...
    assert (
        prof["subscription_allowance"] == 15000
    ), "Max tier allowance should be 15000 cents ($150.00)"
    assert prof["purchased_credits"] == 0
...
def test_mark_welcome_email_sent_and_delete_user_account(sb):
    ...
    sb.table("users").insert(
        {"id": 33, "username": "w", "email": "w@x.com", "credits": 1, "api_key": "kw"}
    ).execute()
...
def test_create_enhanced_user_failure_no_data(sb, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="Failed to create enhanced user"):
        users.create_enhanced_user(
            username="fail_user", email="fail@example.com", auth_method="email"
        )
...
def test_create_enhanced_user_exception_handling(sb, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="Failed to create enhanced user"):
        users.create_enhanced_user(
            username="error_user", email="error@example.com", auth_method="email"
        )
...
def test_get_user_by_username_found(sb):
    ...
    sb.table("users").insert(
        {"id": 555, "username": "findme", "email": "findme@test.com", "credits": 10}
    ).execute()
...
def test_add_credits_to_user_success(sb):
    ...
    sb.table("users").insert(
        {"id": 100, "username": "credittest", "email": "credits@test.com", "credits": 5.0}
    ).execute()
...
def test_add_credits_to_user_exception_handling(sb, monkeypatch):
    ...
    sb.table("users").insert(
        {"id": 101, "username": "erroruser", "email": "error@test.com", "credits": 5.0}
    ).execute()
    ...
    with pytest.raises(Exception, match="Update failed"):
        users.add_credits_to_user(
            user_id=101, credits=10.0, transaction_type="payment", description="Test"
        )
...
def test_deduct_credits_user_not_found(sb):
    ...
    with pytest.raises(RuntimeError, match="User with API key nonexistent_key not found"):
        users.deduct_credits(api_key="nonexistent_key", tokens=10.0, description="Test")
...
def test_record_usage_exception_handling(sb, monkeypatch):
    ...
    users.record_usage(user_id=1, api_key="test_key", model="gpt-4", tokens_used=100, cost=0.5)
...
def test_record_usage_with_latency(sb):
    ...
    sb.table("users").insert(
        {"id": 500, "username": "latency_test", "email": "latency@test.com", "credits": 10}
    ).execute()

    # Just verify the function doesn't crash with latency parameter
    users.record_usage(
        user_id=500, api_key="key_500", model="gpt-4", tokens_used=100, cost=0.5, latency_ms=1500
    )
...
def test_clear_user_cache_specific_key(sb):
    ...
    sb.table("users").insert(
        {"id": 804, "username": "clear_test", "email": "c@test.com", "credits": 5.0}
    ).execute()
    sb.table("api_keys_new").insert(
        {
            "id": 804,
...
def test_invalidate_user_cache(sb):
    ...
    sb.table("users").insert(
        {"id": 807, "username": "inv_test", "email": "inv@test.com", "credits": 5.0}
    ).execute()
    sb.table("api_keys_new").insert(
        {
            "id": 807,
