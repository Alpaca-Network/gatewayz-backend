...
def mock_user():
    """Sample user data"""
    return {"id": 1, "email": "test@example.com", "credits": 100.0, "api_key": "test_api_key_12345"}
...
class TestLogCreditTransaction:
    ...
    def test_log_deduction_transaction_success(
        self, mock_get_client, mock_supabase_client, mock_transaction
    ):
    ...
    def test_log_transaction_with_metadata(self, mock_get_client, mock_supabase_client):
        ...
        result_mock = Mock()
        result_mock.data = [{"id": 1, "user_id": 1, "metadata": metadata}]
        table_mock.insert.return_value.execute.return_value = result_mock
...
class TestGetUserTransactions:
    ...
    def test_get_transactions_basic(self, mock_get_client, mock_supabase_client):
        ...
        # Setup chained mocks for query builder with database-side pagination
        select_mock = Mock()
        eq_mock = Mock()
        order_mock = Mock()
        range_mock = Mock()
        ...
        # Verify query builder calls - uses database-side pagination (default limit 50)
        ...
        order_mock.range.assert_called_once_with(0, 49)  # Default limit 50 -> range(0, 49)
    ...
    def test_get_transactions_with_pagination(self, mock_get_client, mock_supabase_client):
        ...
        # Setup mocks for database-side pagination using .range()
        select_mock = Mock()
        eq_mock = Mock()
        order_mock = Mock()
        range_mock = Mock()
        result_mock = Mock()
        # Database returns only the paginated subset (items 11-20)
        result_mock.data = [{"id": i} for i in range(11, 21)]
        ...
        # Verify - should get items 11-20 (database returns paginated subset)
    ...
    def test_get_transactions_with_type_filter(self, mock_get_client, mock_supabase_client):
        ...
        # Setup mocks for database-side pagination
        select_mock = Mock()
        eq_user_mock = Mock()
        eq_type_mock = Mock()
        order_mock = Mock()
        ...
        result = get_user_transactions(user_id=1, transaction_type=TransactionType.PURCHASE)
...
class TestAddCredits:
    ...
    def test_add_credits_negative_amount(self, mock_get_client):
        ...
        result = add_credits(api_key="test_key", amount=-50.0, description="Invalid")
    ...
    def test_add_credits_zero_amount(self, mock_get_client):
        ...
        result = add_credits(api_key="test_key", amount=0.0, description="Invalid")
    ...
    def test_add_credits_user_not_found(self, mock_get_client, mock_supabase_client):
        ...
        result = add_credits(api_key="invalid_key", amount=50.0, description="Test")
    ...
    def test_add_credits_update_fails(self, mock_get_client, mock_supabase_client, mock_user):
        ...
        result = add_credits(api_key="test_key", amount=50.0, description="Test")
