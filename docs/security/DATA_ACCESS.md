...
|---|---|---|
| `users`, `payments`, `chat_completion_requests`, `rate_limit_usage`, `message_feedback`, `security_audit_log` | RLS enabled, grants revoked (20260527000000) | full access |
| `activity_log`, `api_keys_new`, `credit_transactions`, `coupon_redemptions`, `coupons`, `velocity_mode_events` | RLS enabled, always-true policies dropped (20260527000002); grants revoked for `activity_log`/`api_keys_new`/`credit_transactions` (20260903100000) | full access |
| `usage_records` | RLS enabled, grants revoked, explicit deny policy (20260903100000) | full access |
