...
def reset_welcome_email_sent(user_id: int = None, all_users: bool = False) -> bool:
    ...
    try:
        ...
        if user_id:
            ...
            logger.info(f"Reset welcome_email_sent for all users")
            return True
...
def get_users_without_welcome_emails() -> list:
    ...
    try:
        ...
        result = client.table('users').select('id, username, email, welcome_email_sent').eq('welcome_email_sent', False).execute()
