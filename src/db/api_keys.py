...
def create_api_key(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Encryption unavailable or failed; proceeding without encrypted fields: {enc_e}"
            )
        ...
        try:
            ...
            logger.warning(
                f"Failed to create rate limit config for API key {api_key}: {rate_limit_error}"
            )
...
def validate_api_key_permissions(api_key: str, required_permission: str, resource: str) -> bool:
    ...
    try:
        logger.info(
            f"Validating permissions for API key {api_key[:15]}... - Required: {required_permission} on {resource}"
        )
        ...
        if not key_result.data:
            ...
            logger.info(
                f"API key {api_key[:15]}... not found in api_keys_new, checking legacy table"
            )
            legacy_result = (
                client.table("api_keys")
            ...
            logger.info(
                f"Found in legacy table - is_active: {key_data.get('is_active')}, has is_primary: False (legacy)"
            )
        else:
            key_data = key_result.data[0]
            logger.info(
                f"Found in api_keys_new - is_active: {key_data.get('is_active')}, is_primary: {key_data.get('is_primary', False)}"
            )
        ...
        if key_data.get("is_primary", False):
            ...
            logger.info(
                f"Not a primary key, checking scope_permissions: {key_data.get('scope_permissions', {})}"
            )
        ...
        logger.warning(
            f"Permission denied for {api_key[:15]}... - required: {required_permission} on {resource}, available: {scope_permissions}"
        )
        return False
