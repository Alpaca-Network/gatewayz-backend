...
def create_enhanced_user(
    ...
    try:
        ...
        if not update_result.data:
            logger.warning(
                f"Failed to update users.api_key for user {user_id}, but primary key created successfully in api_keys_new"
            )

        logger.info(
            f"User {user_id} created successfully with primary API key: {primary_key[:15]}..."
        )
...
def add_credits_to_user(
    ...
    try:
        ...
        logger.info(
            f"Added {credits} credits to user {user_id}. Balance: {balance_before} → {balance_after}"
        )
...
def deduct_credits(
    ...
    try:
        ...
        logger.info(
            f"Deducted {tokens} credits from user {user_id}. Balance: {balance_before} → {balance_after}"
        )
