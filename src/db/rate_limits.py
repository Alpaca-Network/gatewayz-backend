    ...
    try:
        ...
        try:
            ...
            if key_record.data:
                ...
                rate_config = client.table('rate_limit_configs').select('*').eq('api_key_id',
                                                                                key_record.data[0]['id']).execute()
...
def check_rate_limit(api_key: str, tokens_used: int = 0) -> dict[str, Any]:
    try:
        ...
        minute_usage = next((u for u in usage_records if
                             u['window_type'] == 'minute' and u['window_start'] == minute_start.isoformat()), None)
        hour_usage = next(
            (u for u in usage_records if u['window_type'] == 'hour' and u['window_start'] == hour_start.isoformat()),
            None)
        day_usage = next(
            (u for u in usage_records if u['window_type'] == 'day' and u['window_start'] == day_start.isoformat()),
            None)
...
def update_rate_limit_usage(api_key: str, tokens_used: int) -> None:
    try:
        ...
        day_start = now.replace(hour=0, minute=0, second=0, microsecond=0).replace(tzinfo=UTC).isoformat()
        ...
        for window_data in [minute_data, hour_data, day_data]:
            try:
                ...
                existing = client.table('rate_limit_usage').select('*').eq('api_key', api_key).eq('window_type',
                                                                                                  window_data[
                                                                                                      'window_type']).eq(
                    'window_start', window_data['window_start']).execute()
...
def get_environment_usage_summary(user_id: int) -> dict[str, Any]:
    ...
    try:
        ...
        keys_result = client.table('api_keys_new').select('environment_tag, requests_used, max_requests').eq('user_id',
                                                                                                             user_id).execute()
...
def get_rate_limit_usage_stats(api_key: str, time_window: str = 'minute') -> dict[str, Any]:
    ...
    try:
        ...
        ).eq('api_key', api_key).gte('created_at', start_time.isoformat()).lt('created_at',
                                                                              end_time.isoformat()).execute()
        ...
        return {
            ...
            'requests_per_second': total_requests / (end_time - start_time).total_seconds() if (
                                                                                                           end_time - start_time).total_seconds() > 0 else 0
        }
...
def get_system_rate_limit_stats() -> dict[str, Any]:
    ...
    try:
        ...
        minute_result = client.table('usage_records').select('api_key, tokens_used').gte('created_at',
                                                                                         minute_ago.isoformat()).execute()
        hour_result = client.table('usage_records').select('api_key, tokens_used').gte('created_at',
                                                                                       hour_ago.isoformat()).execute()
        day_result = client.table('usage_records').select('api_key, tokens_used').gte('created_at',
                                                                                      day_ago.isoformat()).execute()
...
def get_rate_limit_alerts(api_key: str | None = None, resolved: bool = False, limit: int = 100) -> list[
    dict[str, Any]]:
    """Get rate limit alerts with optional filtering"""
    try:
        client = get_supabase_client()

        query = client.table('rate_limit_alerts').select('*').eq('resolved', resolved).order('created_at',
                                                                                             desc=True).limit(limit)
