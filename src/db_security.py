...
def create_secure_api_key(
    ...
    try:
        ...
        raise RuntimeError(f"Failed to create secure API key: {e}") from e
        ...
        for key_data in result.data:
            try:
                ...
                try:
                    ...
                    if key_data["api_key"].startswith(
                        ("gw_live_", "gw_test_", "gw_staging_", "gw_dev_")
                    ):
                ...
                if stored_key == api_key:
                    ...
                    if not key_data.get("is_active", True):
                        audit_logger.log_security_violation(
                            "INACTIVE_KEY", user_id, key_id, f"Key {key_id} is inactive", client_ip
                        )
                        return None
                            ...
                            if expiration_str:
                                ...
                                if expiration < now:
                                    audit_logger.log_security_violation(
                                        ...
                                        user_id,
                                        key_id,
                                        f"Key {key_id} has expired",
                                        client_ip,
                                    )
                                    return None
                        except Exception as date_error:
                            logger.warning(
                                f"Error checking expiration for key {key_id}: {date_error}"
                            )
                    ...
                    if key_data.get("max_requests"):
                        if key_data.get("requests_used", 0) >= key_data["max_requests"]:
                            audit_logger.log_security_violation(
                                ...
                                user_id,
                                key_id,
                                f"Key {key_id} request limit reached",
                                client_ip,
                            )
                            return None
                    ...
                    if referer and key_data.get("domain_referrers"):
                        if not validate_domain_referrers(referer, key_data["domain_referrers"]):
                            audit_logger.log_security_violation(
                                ...
                                user_id,
                                key_id,
                                f"Referer {referer} not in allowlist",
                                client_ip,
                            )
                            return None
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to update last_used_at for key {key_id}: {update_error}"
                        )

                    # Log successful usage
                    audit_logger.log_api_key_usage(
                        user_id, key_id, "api_request", client_ip or "unknown", user_agent
                    )
                ...
                logger.warning(
                    f"Error decrypting key {key_data.get('id', 'unknown')}: {decrypt_error}"
                )
                continue

        # Key not found or validation failed
        audit_logger.log_security_violation(
            "INVALID_KEY", None, None, "Invalid API key provided", client_ip
        )
        return None
...
def rotate_api_key(key_id: int, user_id: int, new_key_name: str = None) -> str | None:
    ...
    try:
        ...
        new_api_key = generate_secure_api_key(key_data.get("environment_tag", "live"))
        ...
        raise RuntimeError(f"Failed to rotate API key: {e}") from e
...
def bulk_rotate_user_keys(user_id: int, environment_tag: str = None) -> dict[str, Any]:
    ...
    try:
        ...
        # Get user's keys
        query = (
            client.table("api_keys_new").select("*").eq("user_id", user_id).eq("is_active", True)
        )
        if environment_tag:
        ...
        audit_logger.log_api_key_creation(
            user_id,
            0,
            f"Bulk rotation ({len(rotated_keys)} keys)",
            environment_tag or "all",
            "admin",
        )

        return {"rotated_count": len(rotated_keys), "new_keys": rotated_keys}

    except Exception as e:
        logger.error(f"Failed to bulk rotate user keys: {e}")
        raise RuntimeError(f"Failed to bulk rotate user keys: {e}") from e
