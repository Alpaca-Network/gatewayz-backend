...
async def get_optimized_gateways_health(
    ...
    auto_fix: bool = Query(
        False, description="Attempt to auto-fix failing gateways when doing live tests."
    ),
    ...
    **Use Cases:**
    ...
    try:
        logger.info(
            f"Gateway health request - live_tests: {include_live_tests}, auto_fix: {auto_fix}"
        )

        result = await get_optimized_gateway_data(
            include_live_tests=include_live_tests, auto_fix=auto_fix
        )

        if not result.get("success"):
            raise HTTPException(
                status_code=503, detail="Gateway health service temporarily unavailable"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error while fetching gateway health"
        )


@router.get("/health/providers/optimized", tags=["health"])
async def get_optimized_providers_health(api_key: str = Depends(get_api_key)):
    """
    ...
    **Returns:**
    ...
    try:
        ...
        if not result.get("success"):
            raise HTTPException(
                status_code=503, detail="Provider health service temporarily unavailable"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error while fetching provider health"
        )
...
async def get_optimized_models_health(
    ...
    **Returns:**
    ...
    try:
        logger.info(
            f"Model health request - gateway: {gateway}, provider: {provider}, status: {status}, limit: {limit}, offset: {offset}"
        )

        result = await get_optimized_models_data(
            gateway=gateway, provider=provider, status=status, limit=limit, offset=offset
        )

        if not result.get("success"):
            raise HTTPException(
                status_code=503, detail="Model health service temporarily unavailable"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error while fetching model health"
        )
...
async def get_optimized_dashboard_data(
    include_live_gateway_tests: bool = Query(
        False, description="Include live gateway latency tests (slower but more accurate)"
    ),
    models_limit: int = Query(
        100, ge=1, le=1000, description="Number of models to include in dashboard"
    ),
    api_key: str = Depends(get_api_key),
):
    """
    ...
    **Returns:**
    ...
    try:
        logger.info(
            f"Dashboard request - live_tests: {include_live_gateway_tests}, models_limit: {models_limit}"
        )
        ...
        gateways_result, providers_result, models_result = await asyncio.gather(
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error while fetching dashboard data"
        )
