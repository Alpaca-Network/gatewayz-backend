...
async def trigger_manual_pricing_sync(admin_user: dict = Depends(require_admin)):
    ...
    **NEW**: This endpoint now returns immediately with a job ID.
    Use GET /admin/pricing/sync/{sync_id} to poll for status and results.
    ...
    **Note**: This now runs asynchronously to avoid Railway's 55-second timeout.
    The sync is queued in the background and you poll for status.

    **Example Response**:
    ```json
    {
        "success": true,
        "sync_id": "abc123-def456",
        "status": "queued",
        "message": "Pricing sync queued for background execution",
        "triggered_by": "admin@example.com",
        "triggered_at": "2026-01-26T12:00:00Z",
        "poll_url": "/admin/pricing/sync/abc123-def456",
        "estimated_duration_seconds": "30-120"
    }
    ```

    **Next Steps**:
    1. Save the `sync_id` from response
    2. Poll GET /admin/pricing/sync/{sync_id} every 5-10 seconds
    3. When `status` is "completed" or "failed", sync is done
    """
    try:
        ...
        sync_id = await queue_background_sync(
            triggered_by=f"admin:{admin_user.get('email', 'unknown')}"
        )

        logger.info(f"Pricing sync queued with ID: {sync_id}")
        ...
        return {
            ...
            "estimated_duration_seconds": "30-120",
            "instructions": "Poll the poll_url endpoint to check sync progress and get results"
        }
...
async def get_pricing_sync_status(
    ...
    **Status Values**:
    ...
    - `success` or `completed`: Sync finished successfully
...
async def get_active_pricing_syncs(admin_user: dict = Depends(require_admin)):
    ...
    This is useful to check if any syncs are currently running before
