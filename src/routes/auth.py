...
def _pay_pending_staking_rewards(address: str, user_id: int) -> None:
    ...
    (staking_rewards pulls in the credits/ledger stack this hot request
    path doesn't otherwise need) and never raises: pay_pending_for_wallet
    itself is written to never raise, but this call site treats a failure
    to even import it the same way -- a signup/login must never fail
    because of a staking-rewards side effect."""
    try:
