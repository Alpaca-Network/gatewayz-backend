...
async def get_optimized_gateway_data(
    include_live_tests: bool = False, auto_fix: bool = False
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        for gateway_name, gateway_info in cached_gateways.items():
            ...
            if models_cache and models_cache.get("data"):
                ...
                models_metadata = {
                    "count": models_count,
                    "last_updated": (
                        _normalize_timestamp(models_cache.get("timestamp")).isoformat()
                        if _normalize_timestamp(models_cache.get("timestamp"))
                        else None
                    ),
                }
            else:
                models_metadata = {"count": 0, "last_updated": None}
        ...
        response = {
            ...
            "summary": {
                ...
                "overall_health_percentage": (
                    round((healthy_count / total_gateways) * 100, 1) if total_gateways > 0 else 0
                ),
            },
        ...
        logger.info(
            f"Gateway data processed in {processing_time:.2f}s for {total_gateways} gateways"
        )
        return response
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "unhealthy": unhealthy_count,
                "overall_health_percentage": (
                    round((healthy_count / tracked_providers) * 100, 1)
                    if tracked_providers > 0
                    else 0
                ),
            },
            "timestamp": datetime.now(UTC).isoformat(),
