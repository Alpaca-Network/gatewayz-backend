...
try:
    ...
    # Using gRPC exporter instead of HTTP due to Tempo compatibility issues
    # HTTP exporter was causing 404 errors even with correct endpoint configuration
...
class OpenTelemetryConfig:
    ...
    def initialize(cls) -> bool:
        ...
        try:
            logger.info("🔭 Initializing OpenTelemetry tracing with gRPC protocol...")

            # Use gRPC endpoint instead of HTTP (more reliable with Tempo)
            # gRPC endpoint format: "tempo.railway.internal:4317" (no http:// prefix)
            ...
            # gRPC endpoints should be in format "host:port" without protocol prefix
            ...
            # For gRPC, determine if connection should be secure (TLS)
            # Railway internal DNS (.railway.internal) should use insecure connection
            ...
            # Create OTLP gRPC exporter with error handling for connection issues
            try:
                ...
                # gRPC exporter automatically appends the correct service path
