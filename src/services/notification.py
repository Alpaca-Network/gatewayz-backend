...
class NotificationService:
    ...
    def create_notification(self, request: SendNotificationRequest) -> bool:
        ...
        try:
            ...
            if request.channel == NotificationChannel.EMAIL:
                success = self.send_email_notification(user_email, request.subject, request.content)
    ...
    def send_subscription_expiry_alert(self, alert_data: dict[str, Any]) -> bool:
        ...
        try:
            user = (
                self.supabase.table("users").select("*").eq("id", alert_data["user_id"]).execute()
            )
            if not user.data:
                return False
            ...
            template_result = (
                self.supabase.table("notification_templates")
                .select("*")
            ...
            variables = {
                ...
                "upgrade_url": "https://gatewayz.ai/plans",  # Update with actual URL
            ...
            text_content = (
                template["text_template"].format(**variables)
