"""DeepInfra model-catalog functions (fetch + normalize).

Moved verbatim from the former per-provider client module when its inference trio
was consolidated onto the OpenAI-compat adapter
(``src/services/providers/openai_compat.py`` / ``adapter_configs.py``).
The pricing-unit conversions below are provider-specific and regression-tested
(tests/services/test_provider_price_units.py) — do not "unify" them.
"""
