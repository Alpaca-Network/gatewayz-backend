...
class ModelHealthMonitor:
    ...
    async def _update_provider_metrics(self):
        ...
        for model_key, health_data in self.health_data.items():
    ...
    def get_model_health(self, model_id: str, gateway: str = None) -> Optional[ModelHealthMetrics]:
        ...
        if gateway:
            ...
            for key, health_data in self.health_data.items():
                if health_data.model_id == model_id:
                    return health_data
            return None
    
    def get_provider_health(self, provider: str, gateway: str = None) -> Optional[ProviderHealthMetrics]:
        ...
        if gateway:
            ...
            # Search across all gateways
            for key, provider_data in self.provider_data.items():
