...
# ============================================================================
# DATABASE-FIRST MODEL FETCHING (Issue #980)
# These functions use database as single source of truth instead of calling
# provider APIs directly. This eliminates duplicate API calls and ensures
# consistency between sync and catalog systems.
# ============================================================================
...
def get_models_from_db(gateway: str | None = None, include_inactive: bool = False) -> list[dict]:
    ...
    Args:
        gateway: Gateway name (e.g., "openrouter", "hug", "groq", "all", None)
                 None or "all" returns models from all providers
...
def get_cached_models(gateway: str = "openrouter"):
    ...
    ⚠️  DEPRECATED: This function calls provider APIs directly, causing duplicate
    API calls and inconsistency with the database. Use get_models_from_db() instead.

    This function is kept for backward compatibility during migration to the
    database-first architecture (Issue #980). It will be removed in a future release.

    Migration guide:
        # Old (calls provider APIs):
        models = get_cached_models("openrouter")

        # New (reads from database):
        models = get_models_from_db("openrouter")

    Args:
        gateway: Gateway name (default: "openrouter")

    Returns:
        List of models from provider API (not database)

    See Also:
        get_models_from_db() - The new database-first approach
    """
    try:
