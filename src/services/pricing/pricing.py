...
def _get_pricing_from_database(model_id: str, candidate_ids: set[str]) -> dict[str, float] | None:
    """Query database for pricing — delegates to the shared resolver.

    Now reads from BOTH the legacy ``model_pricing`` table AND
    ``metadata.pricing_raw`` (where the sync service stores current prices),
    ensuring billing and display always see the same data.
    """
    try:
        from src.services.pricing.pricing_lookup import _resolve_pricing_from_db

        result = _resolve_pricing_from_db(model_id, candidate_ids)
        if result is None:
            logger.debug(
                "[DB MISS] No pricing found for %s (tried %d candidates)",
                model_id,
                len(candidate_ids),
            )
            return None

        prompt_price = max(0.0, float(result["prompt"]))
        completion_price = max(0.0, float(result["completion"]))

        logger.info(
            "[DB SUCCESS] Found pricing for %s: prompt=$%s/token, completion=$%s/token (source=%s)",
            model_id,
            prompt_price,
            completion_price,
        ...
        logger.warning("Database pricing lookup error for %s: %s", model_id, e)
        return None
...
def get_model_pricing(model_id: str) -> dict[str, float]:
    ...
    2. Database (models_catalog table via model sync)
    3. Default pricing ($0.00002 per token)
    ...
    try:
        ...
        # Step 3.5: Try manual pricing JSON (same source that display uses)
        try:
            from src.services.pricing.pricing_lookup import get_model_pricing as _lookup_pricing

            # Infer gateway from model ID prefix (e.g. "openai/gpt-4o" → "openai")
            for candidate in candidate_ids:
                if "/" in candidate:
                    gateway_guess = candidate.split("/")[0]
                    manual = _lookup_pricing(gateway_guess, candidate)
                    if manual:
                        manual_float = {
                            "prompt": max(0.0, float(manual.get("prompt", 0))),
                            "completion": max(0.0, float(manual.get("completion", 0))),
                            "found": True,
                            "source": "manual_json",
                        }
                        with _pricing_cache_lock:
                        ...
                        logger.info("[MANUAL JSON] Using manual pricing for %s", model_id)
                        return manual_float
        except Exception as e:
            logger.warning("Manual pricing JSON lookup failed for %s: %s", model_id, e)
    ...
    try:
        ...
        try:
            ...
            for candidate in candidate_ids:
                if "/" in candidate:
                    gateway_guess = candidate.split("/")[0]
                    manual = await asyncio.to_thread(_lookup_pricing, gateway_guess, candidate)
                    if manual:
                        manual_float = {
                            "prompt": max(0.0, float(manual.get("prompt", 0))),
                            "completion": max(0.0, float(manual.get("completion", 0))),
...
def calculate_cost(model_id: str, prompt_tokens: int, completion_tokens: int) -> float:
    ...
    try:
        ...
        # Apply pricing markup (revenue margin on top of upstream cost)
        from src.config.config import Config

        markup = Config.PRICING_MARKUP
        if markup != 1.0:
            base_cost = total_cost
            total_cost = total_cost * markup
            logger.info(
                f"Cost calculation for {model_id}: "
                f"base=${base_cost:.6f} × {markup}x markup = ${total_cost:.6f} "
