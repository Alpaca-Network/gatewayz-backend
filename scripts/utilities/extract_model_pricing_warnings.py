...
def parse_model_warning_line(line):
    ...
    Expected format:
    ⚠️ Model provider/model-name (normalized: provider/normalized-name) not found in catalog, using default pricing

    Returns:
        dict: Model info with original, normalized, and provider keys, or None if parsing fails
    """
    pattern = r'⚠️\s+Model\s+([^\s]+)\s+\(normalized:\s+([^)]+)\)\s+not found in catalog'
...
✅ No models with missing pricing detected in deployment logs.
...
def main():
    ...
    print("🔍 Model Pricing Warnings Extraction Script")
    print("=" * 50)
    print()
    ...
    sample_log_data = """
2026-01-15 15:16:19    ⚠️ Model alibaba/qwen-3-14b (normalized: alibaba/qwen-3-14b) not found in catalog, using default pricing
2026-01-15 15:16:19    ⚠️ Model google/gemini-2.0-flash (normalized: google/gemini-2.0-flash) not found in catalog, using default pricing
2026-01-15 15:16:19    ⚠️ Model deepseek/deepseek-chat (normalized: deepseek/deepseek-chat) not found in catalog, using default pricing
2026-01-15 15:16:19    ⚠️ Model mistral/mistral-large (normalized: mistral/mistral-large) not found in catalog, using default pricing
2026-01-15 15:16:19    ⚠️ Model meta/llama-3-8b-instruct (normalized: meta-llama/llama-3-8b-instruct) not found in catalog, using default pricing
2026-01-15 15:16:19    ⚠️ Model bfl/flux-1-1-pro (normalized: black-forest-labs/flux-1.1-pro) not found in catalog, using default pricing
2026-01-15 15:16:19    ⚠️ Model bytedance/sdxl-lightning-4step (normalized: bytedance/sdxl-lightning-4step) not found in catalog, using default pricing
2026-01-15 15:16:19    ⚠️ Model cohere/command-r-plus (normalized: cohere/command-r-plus) not found in catalog, using default pricing
    """

    print("📋 Parsing deployment logs...")
    warnings = extract_warnings_from_logs(sample_log_data)

    if warnings:
        print(f"✅ Found {sum(len(m) for m in warnings.values())} models with pricing warnings")
        print(f"📊 Across {len(warnings)} providers")
    else:
        print("✅ No pricing warnings found")
