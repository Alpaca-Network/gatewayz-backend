...
-- ============================================================================
-- This script analyzes if model_id is redundant with model_name
--
-- Run this against your production database:
--   psql $DATABASE_URL -f scripts/verify_model_id_duplication.sql
--
...
\echo ''
\echo '================================================================================'
\echo 'VERIFICATION: Can model_id column be safely removed?'
\echo '================================================================================'
\echo ''
...
\echo '2. EXACT MATCHES (model_id == model_name)'
\echo '--------------------------------------------------------------------------------'
...
\echo ''
\echo 'Examples of exact matches:'
SELECT
    p.slug as provider,
    m.model_id,
    m.model_name
...
\echo '3. SLUGIFIED MATCHES (slugify(model_name) == model_id)'
\echo '--------------------------------------------------------------------------------'
...
\echo ''
\echo 'Examples of slugified matches:'
SELECT
    p.slug as provider,
    m.model_id,
    m.model_name,
...
\echo '4. DIFFERENT VALUES (model_id != model_name)'
\echo '--------------------------------------------------------------------------------'
...
\echo ''
\echo 'Examples of different values (CRITICAL TO REVIEW):'
SELECT
    p.slug as provider,
    m.model_id,
    m.model_name,
...
\echo '5. MULTI-PROVIDER GROUPING ANALYSIS'
\echo '--------------------------------------------------------------------------------'
\echo 'Models offered by multiple providers (grouped by model_id):'
...
\echo ''
\echo 'Models offered by multiple providers (grouped by model_name):'
...
\echo '6. CRITICAL CHECK: Multi-provider grouping discrepancies'
\echo '--------------------------------------------------------------------------------'
\echo 'Models with SAME model_id but DIFFERENT model_names (breaks grouping!):'
