-- Migration: atomic_deduct_credits must write request_id to the COLUMN
--
-- 20260223000003 added p_request_id to this procedure and then inserted it only
-- into the metadata JSONB. The credit_transactions.request_id COLUMN was left
-- NULL on every deduction.
--
-- Consequences, both silent:
--   1. get_transaction_by_request_id() (src/db/credit_transactions.py) filters on
--      the column, so it never matched and the double-charge guard never fired.
--   2. idx_credit_transactions_request_id is a UNIQUE partial index on the
--      column WHERE request_id IS NOT NULL -- always NULL means it never
--      constrained anything either.
--
-- So every credit deduction claimed idempotency protection it did not have: a
-- retried or duplicated request charged the customer again. Measured on
-- production 2026-09-16: 93,090 api_usage rows, 0 with the column populated,
-- while the value was present in metadata the whole time.
--
-- atomic_add_credits (20260705000001) always wrote the column correctly; only
-- the deduction path was affected.
--
-- This re-creates the function with the column populated. Behaviour is otherwise
-- byte-identical, including keeping request_id in metadata so anything reading
-- it there keeps working. Existing rows are left alone: back-filling from
-- metadata could collide with the unique index on values that were never
...
BEGIN
    ...
    INSERT INTO credit_transactions (
        ...
        v_balance_before,
        v_balance_after,
        p_request_id,  -- THE FIX: the column, not just the metadata blob
