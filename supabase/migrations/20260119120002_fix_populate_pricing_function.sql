-- ============================================================================
-- Fix Populate Model Pricing Function
-- ============================================================================
-- Update to use correct column names (no source_gateway column exists)
-- ============================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- Get provider slug (try metadata.source_gateway first, then provider_slug, then top_provider)
        v_provider_slug := COALESCE(
            LOWER(v_model.metadata->>'source_gateway'),
            LOWER(v_model.provider_slug),
            LOWER(v_model.top_provider),
            'unknown'
        );
...
COMMENT ON FUNCTION populate_model_pricing_table() IS
    'Populates model_pricing table with normalized per-token pricing from models table (fixed version)';

-- Log completion
DO $$
BEGIN
    RAISE NOTICE '✅ RPC function updated: populate_model_pricing_table()';
    RAISE NOTICE '   Fixed: Uses correct column names (no source_gateway column)';
