...
--
-- Issue: The frontend uses product ID 'prod_TKOraBpWMxMAIu' for max tier,
-- but the database has 'prod_TKOqRE2L6qXu7s'. This causes tier resolution
-- to fail when processing Stripe webhooks.
--
-- Fix: Add the correct product ID 'prod_TKOraBpWMxMAIu' to subscription_products
...
VALUES ('prod_TKOraBpWMxMAIu', 'max', 'MAX', 150.00, 'Maximum tier with $150 monthly credits', TRUE)
...
BEGIN
    ...
    FROM users
    WHERE stripe_product_id = 'prod_TKOraBpWMxMAIu'
...
WHERE user_id IN (
    SELECT id FROM users WHERE stripe_product_id = 'prod_TKOraBpWMxMAIu'
)
...
BEGIN
    ...
    WHERE u.stripe_product_id = 'prod_TKOraBpWMxMAIu'
