...
-- Note: Only runs if models_catalog table exists (may not exist in all environments)

DO $$
BEGIN
    -- Only proceed if the models_catalog table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'models_catalog') THEN
        ...
        RAISE NOTICE 'Successfully added pricing for 8 models to models_catalog';

    ELSE
        RAISE NOTICE 'Skipping migration: models_catalog table does not exist in this environment';
    END IF;
