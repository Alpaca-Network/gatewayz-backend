-- Migration: Drop Pricing Sync Infrastructure Tables
-- Created: 2026-02-05
-- Phase 4 of Pricing Sync Deprecation
-- Purpose: Clean up tables created for the deprecated pricing sync system
-- Related Issues: #1061, #1062, #1063, #1064

-- ============================================================================
-- IMPORTANT: This is Option A (Recommended)
-- ============================================================================
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE table_name = 'pricing_sync_jobs'
    ) THEN
        RAISE NOTICE 'pricing_sync_jobs table does not exist - skipping';
    END IF;

    RAISE NOTICE 'Starting Phase 4: Pricing Sync Database Cleanup';
    RAISE NOTICE 'Option A: Delete only sync-specific tables, keep model_pricing_history';
END $$;
...
'Historical pricing changes for models with audit trail. Note: Pricing is now synced via model_catalog_sync, not the deprecated pricing_sync system.';
